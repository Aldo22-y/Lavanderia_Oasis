...
<div class="w-full py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800">
        <h1 class="text-2xl font-bold text-white mb-6" data-flux-component="heading">
            Registrar Tipo de Lavado
        </h1>
        <form action="{{ route('admin.tipolavado.store') }}" method="POST" class="space-y-6" data-flux-component="form">
            @csrf

            <!-- Campo Nombre -->
            <div data-flux-field>
                <label for="nombre" class="block text-sm font-medium text-zinc-300 mb-1" data-flux-label>
                    Nombre del tipo de lavado <span class="text-red-500">*</span>
                </label>
                <input type="text" id="nombre" name="nombre"
                    class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 text-white placeholder-zinc-500"
                    placeholder="Ej: Lavado en seco" required maxlength="255" data-flux-control>
                @error('nombre')
            ...
            <div data-flux-field>
                ...
                <input type="number" id="precio" name="precio"
                    class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 text-white placeholder-zinc-500"
                    placeholder="Ej: 15.50" required step="0.01" min="0" data-flux-control>
                @error('precio')
                    <p class="mt-1 text-sm text-red-500 font-medium" data-flux-component="error">{{ $message }}</p>
                @enderror
            </div>

    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-4">
                <a href="{{ route('admin.tipolavado.export-pdf') }}" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                    Exportar PDF
                </a>
                <a href="{{ route('admin.tipolavado.export-excel') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
                    Exportar Excel
                </a>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-zinc-800">
                    @foreach ($tipos_lavado as $lavado)
                        <tr>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ $loop->iteration }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ $lavado->nombre }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">S/ {{ number_format($lavado->precio, 2) }}</td>
                            <td class="px-4 py-4 text-sm text-right">
                                <!-- Botón Editar -->
                                <button @click="openModal({{ $lavado->id }}, '{{ addslashes($lavado->nombre) }}', {{ $lavado->precio }})"
                                    class="text-blue-500 hover:text-blue-400 mr-3">
                                    ✎
                                </button>

                                <!-- Botón Eliminar -->
                                <button onclick="confirmDelete({{ $lavado->id }})" class="text-red-500 hover:text-red-400">
                                    🗑️
                                </button>

                                <!-- Formulario Eliminar -->
                                <form id="delete-form-{{ $lavado->id }}" action="{{ route('admin.tipolavado.destroy', $lavado->id) }}"
        ...
        <div x-show="isOpen" x-cloak x-transition class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 py-12">
                ...
                <div class="relative bg-zinc-900 rounded-lg w-full max-w-md p-8 shadow-xl border border-zinc-800 z-50">
                    <h3 class="text-xl font-bold text-white mb-6">Editar Tipo de Lavado</h3>
                    ...
                    <form :action="'/admin/tipolavado/' + currentId" method="POST">
                        ...
                        <div class="mb-6">
                            <label class="block text-sm text-zinc-300 mb-1">Precio</label>
                            <input type="number" x-model="currentPrecio" name="precio"
                                   class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white"
